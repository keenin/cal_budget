...
The home screen stacks those amounts. Tap either one to see the bills, housing, and card remainders that add up to it. Bills, credit cards, housing, pay, and settings live in the menu at the top right.
...
- **Pay schedule.** Add a schedule for each job: weekly, biweekly, monthly, or every N days, weeks, or months. There is no paycheck amount and no limit on how many schedules you can add. Pick a recent or upcoming payday as the anchor date. You can set an end date or leave it ongoing. The home screen uses the next two paydays across every schedule as the ends of its two windows. The soonest date wins.
...
**Next period** adds unpaid bills and housing due strictly after that payday through the payday after it, plus card remainders due in that same span. That is what the next check needs to cover. A card due after the second payday is in neither number. Paying part of a card subtracts that payment from the window its due date is in. Paying the rest, or more, removes the card. Entering the next statement balance does not keep the old payments.
