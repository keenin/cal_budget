...
The home screen stacks those amounts, each with the dates it covers (for example, Sep 19 – Oct 2). Tap any one to see the bills, housing, and card remainders that add up to it. Tap a row for a short sheet to mark that bill or housing payment paid, undo that, or pay a card. **Paid** on a card applies the full remainder; type an amount and use Record payment for a partial. Edit details in the sheet still opens the full form. The menus open those forms directly. Bills, credit cards, housing, pay, and settings live in the menu at the top right.
...
- **Bills and mortgage / housing.** Same recurrence options, plus a name, amount, and optional notes. Mark the next due date paid if you pay early; that occurrence drops off whichever number it would have been in, and later ones still count. Undo paid clears the latest mark. Turn on **Automatic payment** when the bill is paid on its due date: it stays in the totals until that day, then drops off. Leave it off and it still counts on the due date.
- **Credit cards.** Each card has a name, statement day, when the payment is due (days after the statement, or a day of the month), and the statement balance. Record a payment against that balance if you pay only part of it; the home numbers count what is left. Clear payment undoes that for the current statement. A new statement balance starts with nothing paid. Turn on **Automatic payment** when the statement is paid on its due date: the unpaid remainder stays in the totals until that day, then drops off. Leave it off and a past-due remainder stays in Until payday until you record a payment.
...
**Until payday** adds every unpaid bill and housing occurrence due from today through the next payday, including that payday, plus each credit card’s unpaid remainder due on or before that payday. A card balance that is already past due stays in this number until you pay or update it, unless that card is set to automatic payment. Automatic payment drops a bill, housing occurrence, or card remainder on its due date. If today is a payday, the next payday is the following one, so today does not close this window.
